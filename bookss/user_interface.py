...
def open_user_interface():
    ...
    def load_books(search_query=None):
        ...
        try:
            ...
            if search_query:
                cur.execute("SELECT id, title, author, price FROM books WHERE stock > 0 AND title LIKE ?", ('%' + search_query + '%',))
            else:
                cur.execute("SELECT id, title, author, price FROM books WHERE stock > 0")
    ...
    def place_order():
        try:
            ...
            # Создаем таблицу заказов, если она еще не существует
            ...
            for book in cart_books:
                book_id = book[0]
                cur.execute("INSERT INTO orders (user_id, book_id, quantity, status) VALUES (?, ?, ?, ?)",
                            (current_user, book_id, 1, 'Ожидает обработки'))
    ...
    def show_details():
        ...
        try:
            open_db()
            cur.execute("SELECT title, author, description, price FROM books WHERE id = ?", (book_id,))
            book = cur.fetchone()
            if book:
                ...
                tk.Label(detail_win, text=f"Цена: {book[3]} руб.").pack()
        except sqlite3.OperationalError as e:
            messagebox.showerror("Ошибка", f"Ошибка базы данных:\n{e}")
            print(f"Ошибка базы данных: {e}")
        finally:
